        ...
        <Stagger className="mt-14 grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {SERVICES.map((s) => (
            <StaggerItem key={s.key} className="h-full">
              <a
                href={s.href}
                className="group h-full bg-white border border-line overflow-hidden hover:shadow-[0_18px_40px_-22px_rgba(0,0,0,0.3)] hover:-translate-y-1 transition-all duration-300 flex flex-col"
              >
