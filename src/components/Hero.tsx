...
const categories = [
  { label: "予約効率化", items: ["電話対応の削減", "ペーパレス化でコスト改善", "各種予約の電子化"] },
  { label: "患者管理", items: ["定着・来院数改善", "エンゲージメント向上", "リコール自律促進", "優良患者の発掘"] },
  { label: "データ活用", items: ["経営指標の可視化", "データドリブン経営", "経営判断の迅速化", "キャッシュフロー改善"] },
];
...
function DeviceMockup() {
  ...
  return (
    <div className="relative select-none" style={{ paddingBottom: "68%" }}>
        ...
        >
          <div className="flex items-center gap-2 px-3 py-1.5 bg-[#f0f0f0]">
            ...
            <div className="flex-1 bg-white rounded-md px-2 py-0.5 flex items-center gap-1.5 mx-2">
              ...
              <span className="text-[8px] text-gray-400 truncate">arche.sourirette.jp/admin</span>
            </div>
          </div>
          ...
          <div className="bg-white overflow-hidden" style={{ maxHeight: "180px" }}>
            <div className="flex items-center gap-3 px-3 py-1.5 border-b border-gray-100 bg-gray-50">
              ...
              <div className="flex gap-2 ml-2">
                {["予約管理", "患者管理", "分析"].map((t, i) => (
                  <span
                    key={t}
                    className="text-[7px] px-2 py-0.5 rounded font-medium"
                    style={i === 0 ? { backgroundColor: GREEN, color: "white" } : { color: "#9CA3AF" }}
                  >
                    {t}
                  </span>
                ))}
            ...
            <div className="px-3 space-y-1 py-1">
              {rows.map((r) => (
                ...
                >
                  <span className="text-[7px] font-bold font-mono w-6 shrink-0" style={{ color: r.done ? "#D1D5DB" : GREEN }}>
                    {r.time}
                  </span>
                  <span className="text-[7.5px] font-semibold flex-1 truncate" style={{ color: r.done ? "#9CA3AF" : "#111" }}>
                    {r.name}
                  </span>
                  <span className="text-[6.5px] text-gray-400 hidden sm:block">{r.menu}</span>
                  <span
                    className="text-[6px] px-1 py-0.5 rounded font-medium shrink-0"
                    style={r.done ? { backgroundColor: "#F3F4F6", color: "#9CA3AF" } : { backgroundColor: "#FAF0E0", color: GREEN }}
                  >
                    {r.done ? "完了" : "予定"}
                  </span>
                </div>
            ...
            <div className="mx-3 mt-1 px-2 py-1 rounded text-[6.5px] font-medium" style={{ backgroundColor: "#FAF5ED", color: GREEN }}>
      ...
      {/* スマホ */}
      <div className="absolute bottom-[14px] left-0 w-[30%]" style={{ filter: "drop-shadow(0 8px 24px rgba(0,0,0,0.18))" }}>
        <div className="rounded-[18px] overflow-hidden" style={{ border: "5px solid #1a1a1a", backgroundColor: "#1a1a1a" }}>
  ...
  return (
    <>
      <section
        ...
        style={{ background: "linear-gradient(135deg, #8B7A68 0%, #C4B5A1 50%, #D8CFC5 80%, #E8E3DC80 100%)" }}
      >
        <div className="absolute -top-32 -right-32 w-[600px] h-[600px] rounded-full pointer-events-none" style={{ background: "radial-gradient(circle, rgba(255,255,255,0.07) 0%, transparent 70%)" }} />
        <div className="absolute -bottom-20 -left-20 w-[380px] h-[380px] rounded-full pointer-events-none" style={{ background: "radial-gradient(circle, rgba(255,255,255,0.05) 0%, transparent 70%)" }} />
        <div className="absolute inset-0 pointer-events-none opacity-[0.06]" style={{ backgroundImage: "radial-gradient(circle, #fff 1px, transparent 1px)", backgroundSize: "28px 28px" }} />

        <div className="relative z-10 max-w-site mx-auto px-5 pb-10 pt-8 md:pb-14 md:pt-12">
          <div className="md:max-w-[50%] animate-fadeUp">
            <div className="flex flex-wrap items-center gap-2 mb-5">
              <span className="text-[13px] font-bold px-3 py-[3px] rounded" style={{ backgroundColor: "rgba(255,255,255,0.18)", color: "#fff" }}>
                業務効率化
              </span>
              ...
              <span className="text-[13px]" style={{ color: "rgba(255,255,255,0.9)" }}>で医院を伸ばす</span>
            </div>

            <h1 className="font-black text-white leading-[1.12] tracking-[-0.02em] mb-6" style={{ fontSize: "clamp(32px, 5.5vw, 68px)" }}>
              データで導く、<br />
              スマートな<br />
              歯科経営へ。
            </h1>
            ...
            <div className="flex flex-col sm:flex-row gap-3 md:hidden mb-8">
              <a href={APP_LINKS.demo} className="flex items-center justify-center gap-2 rounded-[14px] px-5 py-3.5 shadow-[0_4px_20px_rgba(0,0,0,0.18)]" style={{ backgroundColor: COLORS.accent }}>
                <Download className="w-4 h-4 shrink-0 text-gray-900" />
                <span className="text-[15px] font-black text-gray-900">デモを見る</span>
              </a>
              <a href={APP_LINKS.signup} className="flex items-center justify-center gap-2 rounded-[14px] px-5 py-3.5 bg-white shadow-[0_4px_20px_rgba(0,0,0,0.18)]">
                <ArrowRight className="w-4 h-4 shrink-0" style={{ color: GREEN }} />
                <span className="text-[15px] font-black text-gray-900">無料で始める</span>
              </a>
            </div>
          </div>
        </div>
        ...
        <div className="hidden md:flex absolute top-[108px] right-[5%] lg:right-[8%] flex-col gap-3 z-20">
          <a href={APP_LINKS.demo} className="flex items-center gap-3 rounded-[18px] px-5 py-3 hover:opacity-95 transition-opacity shadow-[0_4px_20px_rgba(0,0,0,0.18)] min-w-[220px]" style={{ backgroundColor: COLORS.accent }}>
            <div className="flex-1">
              <p className="text-[10px] font-semibold mb-0.5" style={{ color: "#7A5F00" }}>いつでも見られる！</p>
          ...
          <a href={APP_LINKS.signup} className="flex items-center gap-3 rounded-[18px] px-5 py-3 bg-white hover:bg-gray-50 transition-colors shadow-[0_4px_20px_rgba(0,0,0,0.18)] min-w-[220px]">
            <div className="flex-1">
              <p className="text-[10px] font-semibold mb-0.5" style={{ color: GREEN }}>使いやすさを実感！</p>
              <p className="text-[15px] font-black text-gray-900 flex items-center gap-1.5">
                <ArrowRight className="w-4 h-4 shrink-0" style={{ color: GREEN }} />
                無料で始める
              </p>
      ...
      {/* 「ひとつで解決」セクション */}
      <section id="about" className="bg-white py-12 md:py-16 border-b border-gray-100">
              ...
              <div className="space-y-5">
                {categories.map((cat) => (
                  <div key={cat.label} className="flex items-start gap-3">
                    <span className="shrink-0 text-[12px] font-bold text-white px-3 py-[5px] rounded mt-0.5 whitespace-nowrap" style={{ backgroundColor: GREEN }}>
                      {cat.label}
                    </span>
                    <p className="text-[13.5px] text-gray-600 leading-[1.85]">{cat.items.join(" ／ ")}</p>
                  </div>
                ))}
              </div>
            </div>
